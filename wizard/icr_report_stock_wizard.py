...
class IcrReportStock(models.TransientModel):
    ...
    product_ids = fields.Many2many('product.product', 'icr_report_stock_product_rel', 'icr_report_stock_id',
                                   'product_id', 'Product')     # taking product ids from product.product model
    categ_ids = fields.Many2many('product.category', 'icr_report_stock_categ_rel', 'icr_report_stock_id',
                                 'categ_id', 'Categories')      # taking product category ids from product.category
                                                                # model
    location_ids = fields.Many2many('stock.location', 'icr_report_stock_location_rel', 'icr_report_stock_id',
                                    'location_id', 'Locations')  # taking location ids from stock.location model
    ...
    def print_excel_report(self):
        ...
        hours, hours, where_product_ids2, where_location_ids2, where_product_ids2, where_location_ids2,
        where_product_ids, where_location_ids))
